// src/services/trends/genrePersistAndPost.ts
...
import { generateUniqueGenreTrendId } from "./genreTrendService"; // Import the helper
...
export async function createOrUpdateGenreTrend(data: General & { genre_id: string }) { // Accept single genre_id
    try {
        ...
        if (genreTrend) {
            ...
            const uniqueGenreTrendId = await generateUniqueGenreTrendId(data.display_name);

            const newGenreTrendData = {
                genretrend_id: uniqueGenreTrendId, // Use the unique genretrend_id
                genre_id: data.genre_id,
                customer_id: data.customer_id,
                transaction_count: 1,
...
async function postGenreTrendToFreshsales(genreTrend: any, data: General & { genre_id: string }) {
